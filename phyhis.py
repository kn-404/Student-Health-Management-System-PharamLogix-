...
class PhysicalHistory(QWidget):
    ...
    def init_ui(self):
        ...
        title = QLabel("PHYISCAL DETAILS HISTORY")
    ...
    def load_data_from_db(self):
        try:
            ...
            if self.search_name:  # search by student name
                query = "SELECT * FROM physical_details WHERE student_name = %s"
                cursor.execute(query, (self.search_name,))
            else:  # fetch all records
                cursor.execute("SELECT * FROM physical_details")
            ...
            if not data:
                QMessageBox.information(self, "No Data", 
                                        "No physical records found for this student.")
                return
